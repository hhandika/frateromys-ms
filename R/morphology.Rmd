...
var_levels <- c("HB", "TL", "HF", "ER", "WT", "HB_TL")

p1 <- ext |> 
  dplyr::select(species, HB:WT) |> 
  dplyr::filter(species %in% species_group1) |>
  tidyr::drop_na() |>
  dplyr::mutate(HB_TL = WT/HB * 100) |> 
  pivot_longer(cols = HB:HB_TL, names_to = "ext_char", values_to = "measurement") |>
...
skull_clean <- skulls |>
  ...
  dplyr::mutate(across(ZB:DUI, ~ GLS/.x))

skull_pca <- prcomp(skull_clean[, 4:22])
```
